...
const Performance = () => {
  ...
  useEffect(() => {
    ...
    const incompletas = tareas.filter(
      tarea => tarea.fechaVencimiento < moment().format("YYYY-MM-DD") && !tarea.completada
    ).length;
    setTareasIncompletas(incompletas);

    // Calcular tareas pendientes (no completadas y con fecha futura)
    const pendientes = tareas.filter(
      tarea => !tarea.completada && tarea.fechaVencimiento >= moment().format("YYYY-MM-DD")
    ).length;
    ...
    if (completadas === 0) {
      ...
      setConsejos('¡Buen trabajo! Intenta completar más tareas para aumentar tu productividad.');
    } else {
